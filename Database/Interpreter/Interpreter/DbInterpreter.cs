...
namespace DatabaseInterpreter.Core
{
    public abstract class DbInterpreter
    {
        ...
        public string ServerVersion => ConnectionInfo?.ServerVersion;
        ...
        public abstract Task<List<UserDefinedTypeAttribute>> GetUserDefinedTypeAttributesAsync(
        ...
        {
            var attributes = await GetUserDefinedTypeAttributesAsync(filter);
            return SchemaInfoHelper.GetUserDefinedTypes(attributes);
        }

        public virtual async Task<List<UserDefinedType>> GetUserDefinedTypesAsync(DbConnection dbConnection,
            SchemaInfoFilter filter = null)
        {
            var attributes = await GetUserDefinedTypeAttributesAsync(dbConnection, filter);
            return SchemaInfoHelper.GetUserDefinedTypes(attributes);
        }
        ...
        public abstract Task<List<Sequence>> GetSequencesAsync(DbConnection dbConnection,
            SchemaInfoFilter filter = null);
        ...
        public abstract Task<List<Function>> GetFunctionsAsync(DbConnection dbConnection,
            SchemaInfoFilter filter = null);
        ...
        public abstract Task<List<TableColumn>> GetTableColumnsAsync(DbConnection dbConnection,
            SchemaInfoFilter filter = null);
        ...
        public abstract Task<List<TablePrimaryKeyItem>> GetTablePrimaryKeyItemsAsync(DbConnection dbConnection,
            SchemaInfoFilter filter = null);

        public virtual async Task<List<TablePrimaryKey>> GetTablePrimaryKeysAsync(SchemaInfoFilter filter = null)
        {
            var primaryKeyItems = await GetTablePrimaryKeyItemsAsync(filter);
            return SchemaInfoHelper.GetTablePrimaryKeys(primaryKeyItems);
        }

        public virtual async Task<List<TablePrimaryKey>> GetTablePrimaryKeysAsync(DbConnection dbConnection,
            SchemaInfoFilter filter = null)
        {
            var primaryKeyItems = await GetTablePrimaryKeyItemsAsync(dbConnection, filter);
            return SchemaInfoHelper.GetTablePrimaryKeys(primaryKeyItems);
        }
        ...
        public abstract Task<List<TableForeignKeyItem>> GetTableForeignKeyItemsAsync(DbConnection dbConnection,
        ...
        public virtual async Task<List<TableForeignKey>> GetTableForeignKeysAsync(SchemaInfoFilter filter = null,
            bool isFilterForReferenced = false)
        {
            var foreignKeyItems = await GetTableForeignKeyItemsAsync(filter, isFilterForReferenced);
            return SchemaInfoHelper.GetTableForeignKeys(foreignKeyItems);
        }

        public virtual async Task<List<TableForeignKey>> GetTableForeignKeysAsync(DbConnection dbConnection,
            SchemaInfoFilter filter = null, bool isFilterForReferenced = false)
        {
            var foreignKeyItems = await GetTableForeignKeyItemsAsync(dbConnection, filter, isFilterForReferenced);
            return SchemaInfoHelper.GetTableForeignKeys(foreignKeyItems);
        }
        ...
        public abstract Task<List<TableIndexItem>> GetTableIndexItemsAsync(SchemaInfoFilter filter = null,
            bool includePrimaryKey = false);

        public abstract Task<List<TableIndexItem>> GetTableIndexItemsAsync(DbConnection dbConnection,
            SchemaInfoFilter filter = null, bool includePrimaryKey = false);

        public virtual async Task<List<TableIndex>> GetTableIndexesAsync(SchemaInfoFilter filter = null,
            bool includePrimaryKey = false)
        {
            var indexItems = await GetTableIndexItemsAsync(filter, includePrimaryKey);
            return SchemaInfoHelper.GetTableIndexes(indexItems);
        }

        public virtual async Task<List<TableIndex>> GetTableIndexesAsync(DbConnection dbConnection,
            SchemaInfoFilter filter = null, bool includePrimaryKey = false)
        {
            var indexItems = await GetTableIndexItemsAsync(dbConnection, filter, includePrimaryKey);
            return SchemaInfoHelper.GetTableIndexes(indexItems);
        }
        ...
        public abstract Task<List<TableTrigger>> GetTableTriggersAsync(DbConnection dbConnection,
            SchemaInfoFilter filter = null);
        ...
        public abstract Task<List<TableConstraint>> GetTableConstraintsAsync(DbConnection dbConnection,
            SchemaInfoFilter filter = null);
        ...
        public abstract Task<List<Procedure>> GetProceduresAsync(DbConnection dbConnection,
            SchemaInfoFilter filter = null);
        ...
        public abstract Task<List<RoutineParameter>> GetFunctionParametersAsync(DbConnection dbConnection,
            SchemaInfoFilter filter = null);

        public abstract Task<List<RoutineParameter>> GetProcedureParametersAsync(SchemaInfoFilter filter = null);

        public abstract Task<List<RoutineParameter>> GetProcedureParametersAsync(DbConnection dbConnection,
            SchemaInfoFilter filter = null);
        ...
        protected async Task<List<T>> GetDbObjectsAsync<T>(DbConnection dbConnection, string sql)
            ...
            if (!string.IsNullOrEmpty(sql))
                try
                {
                    ...
                    if (isAllOrdersIsZero)
                    {
                        ...
                        objects.ForEach(item => { item.Order = i++; });
                    }
                }
                catch (Exception ex)
                {
        ...
        public virtual async Task<SchemaInfo> GetSchemaInfoAsync(SchemaInfoFilter filter = null)
        {
            ...
            FeedbackInfo("End get schema info.");

            return schemaInfo;
        }

        private bool NeedFetchTableObjects(DatabaseObjectType currentObjectType, SchemaInfoFilter filter,
            string[] childrenNames)
        {
            var filterNames =
                (filter.TableNames ?? Enumerable.Empty<string>()).Union(childrenNames ?? Enumerable.Empty<string>());

            return Option.GetTableAllObjects || NeedFetchObjects(currentObjectType, filterNames, filter);
        }

        private bool NeedFetchObjects(DatabaseObjectType currentObjectType, IEnumerable<string> names,
        ...
        public abstract Task<List<ViewTableUsage>> GetViewTableUsages(SchemaInfoFilter filter,
            bool isFilterForReferenced = false);

        public abstract Task<List<ViewTableUsage>> GetViewTableUsages(DbConnection dbConnection,
        ...
        public abstract Task<List<ViewColumnUsage>> GetViewColumnUsages(DbConnection dbConnection,
            SchemaInfoFilter filter);

        public abstract Task<List<RoutineScriptUsage>> GetRoutineScriptUsages(SchemaInfoFilter filter,
            bool isFilterForReferenced = false, bool includeViewTableUsages = false);

        public abstract Task<List<RoutineScriptUsage>> GetRoutineScriptUsages(DbConnection dbConnection,
        ...
        protected async Task<List<T>> GetDbObjectUsagesAsync<T>(DbConnection dbConnection, string sql)
            where T : DbObjectUsage
        {
            var objects = new List<T>();
        ...
        {
            return InternalExecuteNonQuery(CreateConnection(), new CommandInfo { CommandText = sql });
        }

        public Task<int> ExecuteNonQueryAsync(CommandInfo commandInfo)
        {
            return InternalExecuteNonQuery(CreateConnection(), commandInfo);
        }

        public Task<int> ExecuteNonQueryAsync(DbConnection dbConnection, string sql, bool disposeConnection = false)
        {
            return InternalExecuteNonQuery(dbConnection, new CommandInfo { CommandText = sql }, disposeConnection);
        }

        public Task<int> ExecuteNonQueryAsync(DbConnection dbConnection, CommandInfo commandInfo)
        {
            return InternalExecuteNonQuery(dbConnection, commandInfo, false);
        }
        ...
        public virtual Task<long> GetTableRecordCountAsync(DbConnection connection, Table table,
            string whereClause = "")
        {
        ...
        {
            return GetScalar(CreateConnection(), sql);
        }
        ...
        {
            return await GetScalarAsync(CreateConnection(), sql);
        }
        ...
        {
            return await GetDataReaderAsync(CreateConnection(), sql);
        }
        ...
        public async Task<DataTable> GetPagedDataTableAsync(DbConnection connection, Table table,
            List<TableColumn> columns, string orderColumns, int pageSize, long pageNumber, string whereClause = "")
        {
            ...
            foreach (var column in columns)
            {
                ...
                if (DatabaseType == DatabaseType.MySql)
                {
                    ...
                    if (dataType.Contains("float"))
                    {
                        ...
                        {
                            var strPrecision = dataTypeInfo.Args.Split(',')[0].Trim();
                            int precision;

                            var dataTypeSpec = GetDataTypeSpecification("decimal");
            ...
            var pagedSql = GetSqlForPagination(quotedTableName, strColumnNames, orderColumns, whereClause, pageNumber,
                pageSize);

            var dt = await GetDataTableAsync(connection, pagedSql);
        ...
        public async Task<(long, DataTable)> GetPagedDataTableAsync(Table table, string orderColumns, int pageSize,
            long pageNumber, string whereClause = "", bool isForView = false)
        {
            using (var connection = CreateConnection())
            {
                var total = await GetTableRecordCountAsync(connection, table, whereClause);
                ...
                var columns = await GetTableColumnsAsync(connection, filter);

                var dt = await GetPagedDataTableAsync(CreateConnection(), table, columns, orderColumns, pageSize,
        ...
        public async Task<Dictionary<long, List<Dictionary<string, object>>>> GetPagedDataListAsync(
            DbConnection connection, Table table, List<TableColumn> columns, string primaryKeyColumns, long total,
            long batchCount, int pageSize, string whereClause = "")
        {
            var dictPagedData = new Dictionary<long, List<Dictionary<string, object>>>();

            var pageCount = PaginationHelper.GetPageCount(batchCount, pageSize);
...
*/
        ...
        protected abstract string GetSqlForPagination(string tableName, string columnNames, string orderColumns,
        ...
        public string GetQuotedDbObjectNameWithSchema(DatabaseObject obj)
        {
            ...
            return $"{GetString(obj.Name, true)}";
        }
        ...
        {
            return string.Join(",", columns.OrderBy(item => item.Order).Select(item => GetQuotedString(item.Name)));
        }

        protected bool IsObjectFectchSimpleMode()
        {
            return Option.ObjectFetchMode == DatabaseObjectFetchMode.Simple;
        }

        public string ReplaceSplitChar(string value)
        {
            return value?.Replace(ScriptsDelimiter, ",");
        }
        ...
        public bool IsLowDbVersion(string version, string versionToCompare)
        {
            if (version != null && versionToCompare != null)
            {
                ...
                var versionItemsToCompare = versionToCompare.Split('.');

                var length = Math.Max(versionItems.Length, versionItemsToCompare.Length);

                for (var i = 0; i < length; i++)
                {
                    var item = i < versionItems.Length ? versionItems[i] : "0";
                    var itemToCompare = i < versionItemsToCompare.Length ? versionItemsToCompare[i] : "0";

                    if (!string.IsNullOrEmpty(item) && !string.IsNullOrEmpty(itemToCompare)
                                                    && int.TryParse(item, out _) && int.TryParse(itemToCompare, out _))
                    {
                        var vItem = int.Parse(item);
        ...
        protected string GetFilterNamesCondition(SchemaInfoFilter filter, string[] names, string columnName)
        {
            ...
            {
                var strNames = StringHelper.GetSingleQuotedString(names);
        ...
        {
            if (!ShowBuiltinDatabase)
            {
                var strBuiltinDatabase = BuiltinDatabases.Count > 0
                    ? string.Join(",", BuiltinDatabases.Select(item => $"'{item}'"))
                    : "";
                return string.IsNullOrEmpty(strBuiltinDatabase)
                    ? ""
                    : $"{(isFirstCondition ? "WHERE" : "AND")} {columnName} NOT IN({strBuiltinDatabase})";
            }
        ...
        {
            var isChar = DataTypeHelper.IsCharType(column.DataType);
        ...
        {
            return DataTypeManager.GetDataTypeSpecification(DatabaseType, dataType);
        }

        public virtual bool IsNoLengthDataType(string dataType)
        {
            var dataTypeSpecs = DataTypeManager.GetDataTypeSpecifications(DatabaseType);

            return dataTypeSpecs.Any(item =>
                item.Name.ToUpper() == dataType.ToUpper() && string.IsNullOrEmpty(item.Args));
        }

        public string GetDataTypePrecisionScale(TableColumn column, string dataType)
        {
            var dataTypeSpecification = GetDataTypeSpecification(dataType);

            if (dataTypeSpecification != null)
            {
                var precision = column.Precision.HasValue ? column.Precision.Value : 0;
                var scale = column.Scale.HasValue ? column.Scale.Value : 0;
        ...
        {
            var message =
                $"{state.ToString()}{(state == OperationState.Begin ? " to" : "")} generate script for {StringHelper.GetFriendlyTypeName(dbObject.GetType().Name).ToLower()} \"{dbObject.Name}\".";
            Feedback(FeedbackInfoType.Info, message);
        }
