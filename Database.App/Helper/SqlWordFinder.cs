...
{
    public static List<SqlWord> FindWords(DatabaseType databaseType, string search,
        SqlWordTokenType tokenType = SqlWordTokenType.None, string parentName = null)
    {
        ...
        var schemaInfo = DataStore.GetSchemaInfo(databaseType);

        if (schemaInfo != null)
        {
            if (IsTypeMatched(tokenType, SqlWordTokenType.Schema))
            {
                var owners = schemaInfo.Tables.Where(item => ContainsWithNull(item.Schema, search))
                    .Select(item => item.Schema).Distinct();

                words.AddRange(owners.Select(item => new SqlWord
                    { Type = SqlWordTokenType.Schema, Text = item, Source = item }));
            }
            ...
            if (tokenType == SqlWordTokenType.TableColumn)
            {
                ...
                if (!string.IsNullOrEmpty(search)) columns = columns.Where(item => ContainsWithNull(item.Name, search));
        ...
        {
            var builtinFunctions = FunctionManager.GetFunctionSpecifications(databaseType)
                .Where(item => ContainsWithNull(item.Name, search));
    ...
    private static void FilterDbObjects<T>(List<SqlWord> words, IEnumerable<T> dbObjects,
        SqlWordTokenType currentTokenType, SqlWordTokenType tokenType = SqlWordTokenType.None, string search = null,
        ...
        where T : DatabaseObject
    {
        if (IsTypeMatched(tokenType, currentTokenType))
        {
            ...
            words.AddRange(
                objs.Select(item => new SqlWord { Type = currentTokenType, Text = item.Name, Source = item }));
        }
    }

    public static bool IsTypeMatched(SqlWordTokenType tokenType, SqlWordTokenType currentType)
    {
        if (tokenType == SqlWordTokenType.None || tokenType.HasFlag(currentType)) return true;
        return false;
    }
