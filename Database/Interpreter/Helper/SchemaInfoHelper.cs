...
namespace DatabaseInterpreter.Core
{
    public class SchemaInfoHelper
    {
        ...
        {
            var cloneSchemaInfo =
                (SchemaInfo)JsonConvert.DeserializeObject(JsonConvert.SerializeObject(schemaInfo), typeof(SchemaInfo));
        ...
        public static void MapTableNames(SchemaInfo schemaInfo, Dictionary<string, string> tableNameMappings)
        {
            ...
            schemaInfo.TableIndexes.ForEach(item =>
                item.TableName = GetMappedTableName(item.TableName, tableNameMappings));
            schemaInfo.TableConstraints.ForEach(item =>
                item.TableName = GetMappedTableName(item.TableName, tableNameMappings));
            schemaInfo.TableTriggers.ForEach(item =>
                item.TableName = GetMappedTableName(item.TableName, tableNameMappings));
        ...
        public static void ExcludeDbObjects<T>(List<T> sourceDbObjects, List<T> targetDbObjects)
            where T : DatabaseObject
        {
            if (sourceDbObjects.Count > 0 && targetDbObjects.Count > 0)
            {
                var excludeDbObjects = new List<T>();
        ...
        public static List<TablePrimaryKey> GetTablePrimaryKeys(List<TablePrimaryKeyItem> primaryKeyItems)
        {
            ...
            var groups = primaryKeyItems.GroupBy(item => new
                { item.Schema, item.TableName, item.Name, item.Clustered, item.Comment });

            foreach (var group in groups)
            {
                var primaryKey = new TablePrimaryKey
                {
        ...
        public static List<TableIndex> GetTableIndexes(List<TableIndexItem> indexItems)
        {
            ...
            var groups = indexItems.GroupBy(item => new
            {
                item.Schema, item.TableName, item.Name, item.IsPrimary, item.IsUnique, item.Clustered, item.Type,
                item.Comment
            });
            ...
            foreach (var group in groups)
            {
                ...
                index.Columns.AddRange(group.Select(item => new IndexColumn
                        { ColumnName = item.ColumnName, IsDesc = item.IsDesc, Order = item.Order })
        ...
        public static List<UserDefinedType> GetUserDefinedTypes(List<UserDefinedTypeAttribute> userDefinedTypeAttrs)
        {
            ...
            foreach (var group in groups)
            {
                var userDefinedType = new UserDefinedType
                {
                    Schema = group.Key.Schema,
        ...
        {
            var foreignKeys = new List<TableForeignKey>();

            var groups = foreignKeyItems.GroupBy(item => new
            {
                item.Schema, item.TableName, item.Name, item.ReferencedSchema, item.ReferencedTableName,
                item.UpdateCascade, item.DeleteCascade, item.Comment
            });
            ...
            foreach (var group in groups)
            {
                ...
                foreignKey.Columns.AddRange(group.Select(item => new ForeignKeyColumn
                {
                    ColumnName = item.ColumnName, ReferencedColumnName = item.ReferencedColumnName, Order = item.Order
                }));
        ...
        public static bool IsTableColumnEquals(DatabaseType databaseType, TableColumn column1, TableColumn column2,
            bool excludeComment = true)
        {
            if (IsTableColumnDataTypeAndLengthEquals(databaseType, column1, column2)
                ...
                && ValueHelper.IsStringEquals(StringHelper.GetBalanceParenthesisTrimedValue(column1.DefaultValue),
        ...
        private static bool IsTableColumnDataTypeAndLengthEquals(DatabaseType databaseType, TableColumn column1,
            TableColumn column2)
        {
            ...
            var dataType1 = dataTypeInfo1.DataType;
            var dataType2 = dataTypeInfo2.DataType;
            ...
            var isBytes1 = DataTypeHelper.IsCharType(dataType1);
        ...
        public static void MapDatabaseObjectSchema(SchemaInfo schemaInfo, List<SchemaMappingInfo> mappings)
        {
            ...
            {
                var isAllSourceSchema = string.IsNullOrEmpty(mapping.SourceSchema);
                var targetSchema = mapping.TargetSchema;

                var tables = schemaInfo.Tables.Where(item => item.Schema == mapping.SourceSchema || isAllSourceSchema)
                    .ToList();
                tables.ForEach(item => item.Schema = targetSchema);

                var views = schemaInfo.Views.Where(item => item.Schema == mapping.SourceSchema || isAllSourceSchema)
                    .ToList();
                views.ForEach(item => item.Schema = targetSchema);

                var columns = schemaInfo.TableColumns
                    .Where(item => item.Schema == mapping.SourceSchema || isAllSourceSchema).ToList();
        ...
        {
            return column1.TableName.ToLower() == column2.TableName.ToLower() &&
                   column1.Name.ToLower() == column2.Name.ToLower();
        }
    }
}